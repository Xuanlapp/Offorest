...
contextBridge.exposeInMainWorld('offorestMockup', {
  pickLocalWorkerStorageRoot: () => ipcRenderer.invoke('local-mockup-worker:pick-storage-root'),
  pickLocalWorkerXlapProject: () => ipcRenderer.invoke('local-mockup-worker:pick-xlap-project'),
  readLocalWorkerOutputImage: ({ outputUrl }) => ipcRenderer.invoke('local-mockup-worker:read-output-image', { outputUrl }),
  getLocalWorkerConfig: () => ipcRenderer.invoke('local-mockup-worker:config'),
